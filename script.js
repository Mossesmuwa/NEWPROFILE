...
document.querySelectorAll('.sb-theme-btn, .mob-theme-btn').forEach(btn =>
...
window.addEventListener('pageshow', () => {
  const t = document.querySelector('.page-transition');
  if (t) { t.classList.remove('fade-in'); }
});
...
(function() {
  ...
  const c1 = () => html.getAttribute('data-theme')==='lime' ? '226,255,93' : '139,92,246';
  ...
  (function draw() {
    ...
    if (shift && pts.length > 1) {
      ...
      for (let i = pts.length-1; i >= 0; i--) {
        const p = pts[i]; p.life -= 0.045; p.r *= 0.94;
...
function burst(cx, cy) {
  for (let i = 0; i < 40; i++) {
    const el = document.createElement('div');
    const a = (i/40)*Math.PI*2, d = 60 + Math.random()*70;
...
const revObs = new IntersectionObserver(entries => {
  entries.forEach(e => {
    ...
    const siblings = e.target.parentElement?.querySelectorAll('.reveal,.reveal-l,.reveal-r') || [];
    let delay = 0;
...
function updateCounters() {
  ...
  const deDays    = Math.max(0, Math.floor((now - C.deStart.getTime()) / 86400000));
  document.querySelectorAll('.js-cyber-days').forEach(el => el.textContent = cyberDays);
  document.querySelectorAll('.js-de-days').forEach(el => el.textContent = deDays);
  // Local time
  const t = new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', timeZoneName: 'short' });
  document.querySelectorAll('.js-local-time').forEach(el => el.textContent = t);
}
updateCounters();
...
function glitchGreet(el, text, onDone) {
  ...
  const settle = (idx) => {
    if (idx >= letters.length) { if (onDone) onDone(); return; }
    if (letters[idx] === ' ') { spans[idx].textContent = '\u00a0'; settle(idx+1); return; }
    let count = 0;
...
if (greetEl) {
  ...
  const key = h >= 6 && h < 12 ? 'morning' : h >= 12 && h < 18 ? 'afternoon' : h >= 18 ? 'evening' : 'late';
  // Region aware
  const lang = navigator.language || '';
  const isDE = lang.startsWith('de');
  const deGreet = document.getElementById('greeting-de');
  if (isDE && deGreet) { deGreet.style.display = 'block'; }
  setTimeout(() => glitchGreet(greetEl, greets[key]), loaderShown ? 200 : 2000);
}
...
function typeRole() {
  ...
  if (pause > 0) { pause--; setTimeout(typeRole, 60); return; }
  if (!del) {
    roleEl.textContent = phrase.slice(0, ++ci);
    if (ci === phrase.length) { del = true; pause = 28; }
    setTimeout(typeRole, 75);
  } else {
    roleEl.textContent = phrase.slice(0, --ci);
    if (ci === 0) { del = false; ri = (ri+1) % roles.length; pause = 6; }
...
if (nameEl) {
  nameEl.addEventListener('mouseenter', () => {
    nameEl.innerHTML = '<span style="color:var(--text-1)">Ivan</span><span class="last-name" style="color:var(--text-2);font-weight:300;margin-left:0.2em;"> Kaweesa</span>';
  });
  nameEl.addEventListener('mouseleave', () => { nameEl.textContent = 'Ivan.K'; });
}
...
/* ── FOOTER NAME ── */
document.getElementById('footer-name')?.addEventListener('mouseenter', function() {
  this.style.letterSpacing = '8px';
});
document.getElementById('footer-name')?.addEventListener('mouseleave', function() {
...
const IDLE_ACTIONS = {
  ...
  'contact.html': () => {
    ...
    return () => { if (dot) dot.style.animationDuration = ''; };
  },
};
...
function resetIdle() {
  ...
  if (idleActive && idleCleanup) { idleCleanup(); idleCleanup = null; }
  idleActive = false;
...
document.querySelectorAll('.open-terminal').forEach(el =>
...
document.querySelector('.drag-handle')?.addEventListener('touchmove', e => {
  ...
  w.style.height = Math.max(200, Math.min(startH + dy, innerHeight * 0.95)) + 'px';
});
...
const DE_WORDS = [
  { de: 'die Sicherheit', en: 'security / safety', ex: 'Die Sicherheit ist wichtig.' },
  { de: 'das Netzwerk', en: 'the network', ex: 'Das Netzwerk ist sicher.' },
  { de: 'der Angriff', en: 'the attack', ex: 'Der Angriff wurde abgewehrt.' },
  { de: 'die Verteidigung', en: 'the defence', ex: 'Die Verteidigung war stark.' },
  { de: 'das Passwort', en: 'the password', ex: 'Das Passwort ist verschlüsselt.' },
  { de: 'die Verschlüsselung', en: 'the encryption', ex: 'Verschlüsselung schützt Daten.' },
  ...
  { de: 'die Firewall', en: 'the firewall', ex: 'Die Firewall blockiert Angriffe.' },
  ...
  { de: 'die Ausbildung', en: 'the apprenticeship / training', ex: 'Die Ausbildung dauert drei Jahre.' },
  { de: 'stark', en: 'strong', ex: 'Das Passwort ist stark.' },
  { de: 'die Herausforderung', en: 'the challenge', ex: 'Jede Herausforderung macht stärker.' },
  ...
  { de: 'die Fähigkeit', en: 'the skill / ability', ex: 'Diese Fähigkeit ist wichtig.' },
  ...
  { de: 'der Fortschritt', en: 'the progress', ex: 'Ich sehe meinen Fortschritt.' },
];
function getDEWord() {
...
document.querySelectorAll('.de-word-de').forEach(el => el.textContent = getDEWord().de);
document.querySelectorAll('.de-word-en').forEach(el => el.textContent = getDEWord().en);
document.querySelectorAll('.de-word-ex').forEach(el => el.textContent = getDEWord().ex);

/* ── LIVE SECURITY NEWS ── */
async function loadSecNews(containerId) {
  const el = document.getElementById(containerId);
  if (!el) return;
  const FEEDS = [
    'https://api.rss2json.com/v1/api.json?rss_url=https://feeds.feedburner.com/TheHackersNews&count=4',
    'https://api.rss2json.com/v1/api.json?rss_url=https://www.cisa.gov/news.xml&count=4',
  ];
  const FALLBACK = [
    { title: 'Critical Vulnerability in OpenSSH Allows Remote Code Execution', source: 'The Hacker News', link: 'https://thehackernews.com' },
    { title: 'New Ransomware Variant Targets Healthcare Systems Worldwide', source: 'CISA Advisory', link: 'https://cisa.gov' },
    { title: 'Researchers Discover Zero-Day in Popular VPN Software', source: 'Security Week', link: 'https://securityweek.com' },
    { title: 'Nation-State Actors Exploit Unpatched Exchange Servers', source: 'Krebs on Security', link: 'https://krebsonsecurity.com' },
  ];
  try {
    ...
    data.items.slice(0,4).forEach((item, i) => {
      const timeAgo = item.pubDate ? getTimeAgo(new Date(item.pubDate)) : 'Recent';
      ...
      div.innerHTML = `
        <a href="${item.link}" target="_blank" rel="noopener" class="news-link">
          <div class="news-source">${item.author || 'The Hacker News'} · ${timeAgo}</div>
          <div class="news-title">${item.title}</div>
          <span class="news-arrow">↗</span>
        </a>`;
      el.appendChild(div);
    });
    el.querySelectorAll('.reveal').forEach(r => setTimeout(() => r.classList.add('v'), 100));
  } catch {
    el.innerHTML = '';
    FALLBACK.forEach((item, i) => {
      ...
      div.innerHTML = `
        <a href="${item.link}" target="_blank" rel="noopener" class="news-link">
          <div class="news-source">${item.source} · Recent</div>
          <div class="news-title">${item.title}</div>
          <span class="news-arrow">↗</span>
        </a>`;
      el.appendChild(div);
...
/* ── CVE OF THE DAY ── */
const CVES = [
  { id: 'CVE-2024-3400', severity: 'CRITICAL', product: 'Palo Alto PAN-OS', desc: 'Command injection in GlobalProtect gateway allows unauthenticated RCE.' },
  { id: 'CVE-2024-21762', severity: 'CRITICAL', product: 'Fortinet FortiOS', desc: 'Out-of-bounds write vulnerability allows unauthenticated remote code execution.' },
  { id: 'CVE-2023-46604', severity: 'CRITICAL', product: 'Apache ActiveMQ', desc: 'Allows remote code execution via specially crafted ClassInfo OpenWire protocol message.' },
  { id: 'CVE-2023-44487', severity: 'HIGH', product: 'HTTP/2 Protocol', desc: 'Rapid Reset Attack enables large-scale DDoS by resetting streams rapidly.' },
  { id: 'CVE-2024-1709',  severity: 'CRITICAL', product: 'ConnectWise ScreenConnect', desc: 'Authentication bypass allows creation of admin accounts without credentials.' },
  { id: 'CVE-2022-30190', severity: 'HIGH', product: 'Microsoft MSDT (Follina)', desc: 'Remote code execution via malicious Office documents without macros.' },
  { id: 'CVE-2021-44228', severity: 'CRITICAL', product: 'Apache Log4Shell', desc: 'JNDI injection in logging library affects millions of Java applications worldwide.' },
];
function getCVE() { return CVES[Math.floor(Date.now()/86400000) % CVES.length]; }
window.getCVE = getCVE;
const cve = getCVE();
document.querySelectorAll('.cve-id').forEach(el => el.textContent = cve.id);
document.querySelectorAll('.cve-product').forEach(el => el.textContent = cve.product);
document.querySelectorAll('.cve-desc').forEach(el => el.textContent = cve.desc);
document.querySelectorAll('.cve-severity').forEach(el => {
  el.textContent = cve.severity;
  el.className = 'cve-severity chip ' + (cve.severity === 'CRITICAL' ? 'chip-red' : 'chip-amber');
});
...
const CMDS = {
  ...
  'uname -a': () => [`<span class="t-cyan">IVAN.SYS 2.0 — Curiosity, Kali Linux, Deutsch lessons. Kernel: handcrafted.</span>`],
  history: () => {
    const h = [...cmdHist].slice(0,8);
    return h.length ? h.map((c,i) => `<span class="t-dim">${i+1}</span>  ${c}`) : [`<span class="t-dim">no history</span>`];
  },
  germany: () => [
  ...
  hire: () => {
    ...
    return [`<span class="t-lime">Good initiative. Routing to secure channel...</span>`];
  },
  flag: () => {
    addFlag(3);
    return [
  ...
  'cat flag.txt': () => [`<span class="t-white">FLAG_03/05 {t3rm1n4l_3xpl0r3r}</span>`, `<span class="t-red">████_04/05 {████████}</span>`, `<span class="t-red">████_05/05 {████████}</span>`],
  'ls /secret/': () => [
  ...
  matrix: () => { matrixRain(); return [`<span class="t-dim">Initialising...</span>`]; },
  'sudo rm -rf /': () => [
  ...
  exit: () => {
    setTimeout(() => {
      ...
      setTimeout(() => {
        openTerminal();
        termPrint(`<span class="t-lime">You can check out any time you like...</span>`);
        termPrint(`<span class="t-dim">...but you can never leave.</span>`, 600);
      }, 500);
    }, 600);
...
async function runCmd(raw) {
  ...
  if (!cmd) return;
  cmdHist.unshift(cmd); if (cmdHist.length > 20) cmdHist.pop();
  histIdx = -1;
  await termPrint(`${tPrompt()}&nbsp;<span class="t-white">${raw.trim()}</span>`);
  const fn = CMDS[cmd];
  if (fn) {
    const lines = fn();
    ...
    if (/^[a-z]+$/.test(cmd) && cmd.length > 1) {
      ...
      await termPrint(`<span class="t-lime">Hello, ${name}. Ivan is expecting you.</span>`, 30);
    } else {
      await termPrint(`<span class="t-red">command not found: ${cmd}</span>`, 30);
      await termPrint(`<span class="t-dim">Type <span class="t-lime">help</span> for commands</span>`, 60);
    }
    await termPrint('&nbsp;', 90);
  }
}
...
termInput?.addEventListener('keydown', e => {
  ...
  else if (e.key === 'ArrowUp') { e.preventDefault(); histIdx = Math.min(histIdx+1, cmdHist.length-1); termInput.value = cmdHist[histIdx]||''; }
  else if (e.key === 'ArrowDown') { e.preventDefault(); histIdx = Math.max(histIdx-1,-1); termInput.value = histIdx>=0?cmdHist[histIdx]:''; }
});
termInput?.addEventListener('focus', () =>
  setTimeout(() => termInput.scrollIntoView({ behavior:'smooth', block:'center' }), 300)
);
...
function matrixRain() {
  ...
  let n = 0;
  const iv = setInterval(() => {
    const line = Array.from({length:38},()=>ch[Math.floor(Math.random()*ch.length)]).join('');
    termPrint(`<span style="color:#00ff9d;opacity:${0.3+Math.random()*0.7}">${line}</span>`);
    if (++n >= 18) { clearInterval(iv); setTimeout(() => runCmd('clear'), 200); }
  }, 80);
}

/* ── COMMAND PALETTE ── */
const CMD_DATA = [
  { ico:'ti-home',        name:'Home',          desc:'Back to the start',                url:'../index.html' },
  { ico:'ti-user',        name:'About',          desc:'Who is Ivan?',                     url:'about.html' },
  { ico:'ti-radar',       name:'Arsenal',        desc:'Skills, tools, certifications',    url:'arsenal.html' },
  { ico:'ti-activity',    name:'Operations',     desc:'Lab writeups and projects',        url:'operations.html' },
  { ico:'ti-chart-bar',   name:'Intel',          desc:'Why cybersecurity matters',        url:'intel.html' },
  { ico:'ti-mail',        name:'Contact',        desc:'Open a secure channel',            url:'contact.html' },
  { ico:'ti-terminal-2',  name:'Open Terminal',  desc:'Launch the Kali terminal',         url:'#', fn:()=>{closeCMD();openTerminal();} },
  { ico:'ti-user-check',  name:'Hire Ivan',      desc:'Pre-fill contact form',            url:'contact.html?subject=I+want+to+work+with+you' },
  { ico:'ti-flag',        name:'Hunt Flags',     desc:'5 flags hidden on this site',      url:'#', fn:()=>{closeCMD();openTerminal();} },
];
...
let cmdFiltered = CMD_DATA, cmdSel = 0;

function openCMD() { cmdOv?.classList.add('open'); cmdInp?.focus(); renderCMD(''); }
function closeCMD() { cmdOv?.classList.remove('open'); if(cmdInp) cmdInp.value=''; }
...
document.addEventListener('keydown', e => {
  ...
  if (e.key==='Escape') { closeTerminal(); closeCMD(); closeDrawer(); }
...
cmdInp?.addEventListener('keydown', e => {
  if (e.key==='ArrowDown') { e.preventDefault(); cmdSel=Math.min(cmdSel+1,cmdFiltered.length-1); renderCMD(cmdInp.value,true); }
  if (e.key==='ArrowUp')   { e.preventDefault(); cmdSel=Math.max(cmdSel-1,0); renderCMD(cmdInp.value,true); }
  if (e.key==='Enter') selectCMD(cmdSel);
});

function renderCMD(q, keepSel=false) {
  if (!keepSel) cmdSel = 0;
  cmdFiltered = CMD_DATA.filter(d => !q ||
    d.name.toLowerCase().includes(q.toLowerCase()) ||
    d.desc.toLowerCase().includes(q.toLowerCase())
  );
  if (!cmdRes) return;
  if (!cmdFiltered.length) { cmdRes.innerHTML='<div class="cmd-empty">No results</div>'; return; }
  cmdRes.innerHTML = cmdFiltered.map((d,i) => `
  ...
  cmdRes.querySelectorAll('.cmd-result').forEach((el,i) => {
    el.addEventListener('mouseenter', () => { cmdSel=i; renderCMD(cmdInp?.value||'',true); });
...
console.log('%c// there are 5 flags on this site. good luck.', 'color:#4a6080;font-family:monospace');
addFlag(1);
...
window.ivan = {
  ...
  help: () => console.log('Available: ivan.unlock(), ivan.flags(), ivan.contact()'),
  flags: () => {
    const f = JSON.parse(localStorage.getItem('iv-flags')||'[]');
    console.log(`%cFlags found: ${f.length}/5`, 'color:#e2ff5d;font-family:monospace');
    console.log(`%cFound: ${f.join(', ')||'none yet'}`, 'color:#4a6080;font-family:monospace');
  },
  contact: () => console.log(`%cEmail: ${C.email}`, 'color:#e2ff5d;font-family:monospace'),
};
...
document.addEventListener('contextmenu', e => {
  ...
  const item = (icon, text, fn) => {
    ...
    d.addEventListener('mouseenter', () => d.style.background = 'var(--bg-3)');
    d.addEventListener('mouseleave', () => d.style.background = '');
    d.addEventListener('click', () => { m.remove(); fn(); });
    return d;
  };
  m.appendChild(item('ti-code', 'View source', () => window.open(location.href)));
  m.appendChild(item('ti-copy', 'Copy URL', () => navigator.clipboard.writeText(location.href)));
  const sep = document.createElement('div');
  sep.style.cssText = 'height:0.5px;background:var(--border);margin:4px 0';
  m.appendChild(sep);
  const flagItem = item('ti-flag', `Hunt flags <span style="color:var(--text-3);font-size:0.72rem;margin-left:auto">${found}/5</span>`, openTerminal);
...
document.addEventListener('keydown', e => {
  ...
  if (konamiSeq.join(',') === KONAMI.join(',')) {
    ...
    if (konamiCount === 1) {
      ...
      setTimeout(async () => {
        await termPrint(`<span class="t-lime">[ivan]: I see you've been here before.</span>`);
        await termPrint(`<span class="t-dim">That's the Konami code. Try something harder.</span>`, 400);
        await termPrint(`<span class="t-dim">Hint 1: read the source code.</span>`, 800);
        await termPrint(`<span class="t-dim">Hint 2: visit /humans.txt</span>`, 1200);
        await termPrint(`<span class="t-dim">Hint 3: ivan.unlock() in the console.</span>`, 1600);
      }, 400);
    }
  }
});
...
function breach() {
  ...
  let i = 0; bt.innerHTML = '';
  const iv = setInterval(() => {
    bt.innerHTML += (lines[i]||'') + '<br>';
    if (++i >= lines.length) {
      clearInterval(iv);
      setTimeout(() => { ov.classList.remove('active'); document.body.style.overflow=''; }, 1200);
    }
  }, 300);
...
function addFlag(n) {
  ...
  document.querySelectorAll('.flag-count').forEach(el => el.textContent = stored.length);
}
...
function showDebrief() {
  ...
  if (d) { d.style.display='flex'; if(navigator.vibrate) navigator.vibrate([100,50,100]); }
}
...
document.addEventListener('keydown', e => {
  ...
  map.innerHTML = `<div style="background:var(--bg-2);border:0.5px solid var(--border-hi);
    ...
    ${[['Ctrl+K','Command palette'],['T','Open terminal'],['?','This menu'],['Esc','Close everything'],
       ['↑↑↓↓←→←→BA','...you know what this does']
    ].map(([k,v])=>`<div style="display:flex;justify-content:space-between;gap:2rem;padding:6px 0;border-bottom:0.5px solid var(--border);font-size:0.8rem;">
      <span style="color:var(--accent)">${k}</span><span style="color:var(--text-2)">${v}</span></div>`).join('')}
...
window.addEventListener('beforeprint', () => {
  ...
  qr.id = 'print-qr';
  qr.style.cssText = 'display:none;text-align:center;padding:20px;font-family:monospace;font-size:12px;';
...
window.addEventListener('afterprint', () => document.getElementById('print-qr')?.remove());
...
document.getElementById('contact-form')?.addEventListener('submit', async function(e) {
  ...
  if (!name||!email||!msg) {
    showFeedback(fb, 'Please fill in all required fields.', 'error'); return;
  }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    showFeedback(fb, 'Please enter a valid email address.', 'error'); return;
  }
  // Save to localStorage in case of navigation
  localStorage.setItem('iv-form-draft', JSON.stringify({ name, email, msg }));
  btn.disabled = true;
  btn.innerHTML = '<i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i> Transmitting...';
  try {
    const res = await fetch(C.formspree, {
      method:'POST',
      headers:{'Content-Type':'application/json','Accept':'application/json'},
      body: JSON.stringify({ name, email, subject: document.getElementById('cf-subject')?.value, message: msg })
    });
    if (res.ok) {
...
if (draft) {
  if (document.getElementById('cf-name')) document.getElementById('cf-name').value = draft.name||'';
  if (document.getElementById('cf-email')) document.getElementById('cf-email').value = draft.email||'';
  if (document.getElementById('cf-message')) document.getElementById('cf-message').value = draft.msg||'';
}

// Real-time validation
document.getElementById('cf-email')?.addEventListener('input', function() {
  const valid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(this.value);
  this.style.borderColor = this.value.length > 3 ? (valid ? 'var(--green)' : 'var(--red)') : '';
  this.style.boxShadow   = this.value.length > 3 ? (valid ? '0 0 0 3px rgba(0,255,157,0.1)' : '0 0 0 3px rgba(255,59,92,0.1)') : '';
});
